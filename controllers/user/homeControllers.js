  ...
  try {
    ...
    if (category) {
      ...
      if (Array.isArray(category)) {
        ...
        categoryNames = category.includes(",")
    ...
    if (priceRange) {
      ...
      if (Array.isArray(priceRange)) {
        ...
        priceFilters = priceRange.includes(",")
          ? priceRange.split(",").map((filter) => filter.trim())
          : [priceRange.trim()];
      }
...
const getProductDetails = async (req, res) => {
  try {
    ...
    relatedProducts = relatedProducts.map(p => {
      ...
      const pOffer = Number(p.productOffer) || 0;
      const cOffer = Number(p.category?.categoryOffer) || 0;

      const max = Math.max(pOffer, cOffer);
      const disc = Math.round((base * max) / 100);
      const final = base - disc;

      return {
        ...p.toObject(),
        appliedOffer: isNaN(max) ? 0 : max,
        discountedPrice: isNaN(final) ? base : final,
      };
    });
